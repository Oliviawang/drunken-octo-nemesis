...
function Item(p,a,item)
{
	...
	this.draw = function()
	{
		...
		{
			if (this.images[i].t==0 && this.images[i].a>this.a) break;
		}
		
		var f = Math.sqrt(this.a/this.images[i].a);
		var w = f * this.images[i].w;
		var h = f * this.images[i].h;
		
		this.o = R.image(this.images[i].url, p[0]-w/2, p[1]-h/2, w, h);
	...
	this.resize = function(a)
	{
		...
		{
			i=j;
			if (this.images[j].t==0 && this.images[j].a>a) break;
		}
		
		if (typeof(this.images[i])=="undefined") console.log(i, a, this.item, this.images[i]);
		
		var f = Math.sqrt(a/this.images[i].a);
		var w = f * this.images[i].w;
		var h = f * this.images[i].h;
		
		this.o.attr({src: this.images[i].url, x: p[0]-w/2, y: p[1]-h/2, width: w, height: h});
