...
function View(basediv, topdiv, data)
{
	...
	this.init = function()
	{
		...
		/* this.st = R.set();
		   var data = [[60000, 60000, 40000]];
	        var labels = [["Paris", "London", "New York"]];  
	        var cordinate=  Treemap.generate(data,this.width, this.height);
	          for(i=0;i<cordinate.length;i++){
	        	 
	        	
	        		// alert(labels[i][j]);
	        			var x11=cordinate[i][0];     	x11=parseFloat(x11);
	        		var y11=cordinate[i][1];          y11=parseFloat(y11);
	        		var x12=cordinate[i][2];         x12=parseFloat(x12);
	        		var y12=cordinate[i][3];         y12=parseFloat(y12);
	        		
	        		var w=x12-x11;
	        		var h=y12-y11;
	        		
	        		this.bg1=R.rect(x11, y11, w, this.h).attr({opacity: 0});
	        		st.push(this.bg1);
	        	 }*/
	...
	this.changeBase = function()
	{
		...
		$("."+basediv).attr({id: 'oldbase'}).css({'z-index': 0});   //  base old layer
		$('body').append('<div class="'+basediv+'" id="'+basediv+'"></div>');  // base new layer
		$("#"+basediv).css({opacity: 0});
		//$("#"+basediv).css({'z-index': 1});
		if (this.facet!=1)
		{
		...
		else if(this.facet==3){
			
			this.base = new TreeMap(basediv, this).init();
		}
	};
	...
	this.map = function(item, old)
	{
		...
		if (this.facet==0)
		{
			...
			var y =	this.interval(
				item.pop,
				this.maxPop, this.minPop,
				100, $("#"+topdiv).height()-50
			);
			var y = this.height/2;
	...
	this.window = function()
	{
		...
		this.missing = [];
var n;
		if (typeof NMAX !== "undefined")  n = NMAX;
		else  n = this.items.length;		
		n = Math.min(this.items.length, n);
		
		this.minPop = 100;  // min pop up items?
		this.maxPop = 0;
		
		for (var i=0; i < n; i++)
		{			
			if (this.hasFacet(this.items[i])==false) this.missing.push(i);
			else if (this.within(this.items[i])) {
				this.shown.push(i);
				this.minPop = Math.min(this.minPop, this.items[i].pop);
	 			this.maxPop = Math.max(this.maxPop, this.items[i].pop);
			}
			...
			this.changeItem = function(it, animate)
			{
				...
				if (animate)
				{
					...
					if (d.ia > d_.ia) 
					{
						...
						if (typeof it === "undefined" && typeof it.obj === "undefined")
	...
	this.hasFacet = function(item)
	{
		...
		else if (this.facet==3 && item.mdsx != null && typeof item.mdsy != null) return true;
		else return false;
	};
...
function TimeMap(div, view, start, end)
{
	...
	this.draw = function(animate)
	{
		...
		var data = [60000, 60000, 40000];
	       // var labels = [["Paris", "London", "New York"]];  
	        var cordinate=  Treemap.generate(data,this.width,
	        		this.height);
	          for(var m=0;m<cordinate.length;m++){
	        	 
	        	
	        		// alert(labels[i][j]);
	        			var x11=cordinate[m][0];     	x11=parseFloat(x11);   console.info("x11"+x11);
	        		var y11=cordinate[m][1];          y11=parseFloat(y11);
	        		var x12=cordinate[m][2];         x12=parseFloat(x12);
	        		var y12=cordinate[m][3];         y12=parseFloat(y12);
	        		 var w=x12-x11;
	        		 var h=y12-y11;
	        		
	        		var x = this.map_(x11)+this.margin/2;
	        		var x12=this.map_(w)+this.margin/2;
	    			var w = this.map_(x12-)-this.margin/2 - x; if (w<0) w = 0;
	    			var y = top_margin[1+this.level-l];
	    			var h = $("#"+div).height() - y - bot_margin;
	        		
	    			
	    			
	        		var path="M "+x11+" "+y11+"L"+x12+" "+y11+" L "+x12+" "+y12+"L"+x11+" "+y12+"L"+x11+" "+y11+" Z";    console.info("path_rect?"+path);
	        		var attr = {fill: '#ccc', opacity: 1, stroke: '#000'};
	        		
	        		var id = (this.level-1)+"_"+m;
	    			if (typeof this.regions[id] == "undefined") this.regions[id] = {l:this.level-1, i:m}; 
		...
		*/
		...
		for (id in this.regions)
		{
			...
			// if (isNaN(x) || isNaN(y) || isNaN(w) || isNaN(h)) console.log(i, this.map(i), this.margin);
	...
	this.map = function(time, old)
	{
		...
		if (old)
		{
			var p = view.interval(time, this.start_old, this.end_old, 0, this.width);

		}
		else
		{
			var p = view.interval(time, this.start, this.end, 0, this.width);
		}
		
		// if (isNaN(p)) console.log(time, old, this.start_old, this.end_old, this.start, this.end, this.width);
	...
	this.zoom = function(e,d)
	{
		...
		else var span = span_old * scale; // zoom out
		...
		if (isNaN(this.start)||isNaN(this.end)) console.log(f, x, w, span, span_old, scale, d, D, SPANMIN, SPANMAX);
...
function TagMap(div, view)
{
		...
		/* var data = [[60000, 60000, 40000]];
		 ...
		 var cordinate=  Treemap.generate(data,this.width, this.height);
	          ...
	          for(var i=0;i<cordinate.length;i++){
	        			...
	        			var x11=cordinate[i][0];     	x11=parseFloat(x11);   console.info("x11"+x11);
	        		var y11=cordinate[i][1];          y11=parseFloat(y11);
	        		var x12=cordinate[i][2];         x12=parseFloat(x12);
	        		var y12=cordinate[i][3];         y12=parseFloat(y12);
	        		...
	        		//this.bg1=R.rect(x11, y11, w, h).attr({fill: this.colors[0], stroke: '#000'});
	        		
	        		this.st.push(this.bg1);
	        	 }*/
	...
	{	
		...
		for (var c=0; c < 2; c=c+1)
		{
			...
			{				
				...
				{
					p = this.map(path_array[j], path_array[j+1]);  console.info("path_array"+path_array[j]);
					pixels[j] = p[0];
					pixels[j+1] = p[1];
				}
				
				var l = pixels.length;				
				var path = "M "+pixels[l-2]+" "+pixels[l-1]+" C "+pixels.join(" ")+" Z";    console.info("path_dix?"+path);
		...
		console.info(this.paths);
	/*var width = this.right-this.left;
	 var height = this.bottom-this.top;
		var data = [60000, 60000, 40000];
	       // var labels = [["Paris", "London", "New York"]];  
	        var cordinate=  Treemap.generate(data,-0.013486614141478781,
	        		-0.015487014221494788);
	          for(var m=0;m<cordinate.length;m++){
	        		...
	        		var path="M "+x11+" "+y11+"L"+x12+" "+y11+" L "+x12+" "+y12+"L"+x11+" "+y12+"L"+x11+" "+y11+" Z";    console.info("path_rect?"+path);
	        		var attr = {fill: '#ccc', opacity: 1, stroke: '#fff','stroke-width':'2px'};
	        		//this.bg1=R.rect(x11, y11, w, h).attr({fill: this.colors[0], stroke: '#000'});
	        		if (typeof this.paths["c_"+m] === "undefined" )
					{
						this.paths["c_"+m] = R.path(path).attr(attr);		
						
					}
					else
					{
						if (animate) this.paths["c_"+m].animate({path: path}, SPEED);
						else this.paths["c_"+m].attr({path: path});
					}
	        		//this.st.push(this.bg1);
	        	 }*/
		// labels
		//console.info("final"+this.paths.r_0);
		return;
	...
	this.pan = function(x, y)
	{
		...
		for (i in this.paths) this.paths[i].translate(x, y);   console.info("path"+ this.paths[i].x);
...
function TreeMap(div, view)
{
	...
	var width = this.right-this.left;
	 ...
	 var w=$(window).width();
	 var h=$(window).height();
	 var R1 = Raphael(0,0,w,h);
		var data = [60000, 60000, 40000];
	       // var labels = [["Paris", "London", "New York"]];  
	        var cordinate=  Treemap.generate(data,0.06,-0.05);
	          for(var m=0;m<cordinate.length;m++){
